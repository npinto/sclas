...
def kernel_generate_fromcsv(input_csv_fname,
    ...
    if nowhiten:
        ...
        #corrcoef_kernels = {}
        corrcoef_kernels = sp.zeros((ncat,) + featshape, dtype='float32')

        error = False    
        for itrain, fname in enumerate(train_fnames):
            try:
            ...
            #fvector = fvector.astype('float64')

            if corrcoef_type == 'pos_mean':
                icat = cat_index[train_labels[itrain]]
                
                corrcoef_kernels[icat] += fvector.copy()

            else:
                for icat, cat in enumerate(categories):                

                    if corrcoef_type == 'pos_neg_mean':                    
                        corrcoef_ker = ((train_labels[itrain]==cat)*2-1) * fvector
                        corrcoef_ker /= ntrain
    ...
    #                     corrcoef_ker = train_features[train_labels == cat].mean(0)
                    ...
                    corrcoef_kernels[icat] += corrcoef_ker                    
            
            pbar.update(itrain+1)
        ...
        print "normalizing corrcoef_kernels"
        for icat in xrange(corrcoef_kernels.shape[0]):
            corrcoef_ker = corrcoef_kernels[icat]
        ...
        for icat, cat in enumerate(categories):
            ...
            if corrcoef_type == 'pos_neg_mean':
                corrcoef_ker = train_features[train_labels == cat] \
                               - train_features[train_labels != cat]
                corrcoef_ker = corrcoef_ker.mean(0)
            elif corrcoef_type == 'pos_mean_neg_mean':
                corrcoef_ker = train_features[train_labels == cat].mean(0) \
                               - train_features[train_labels != cat].mean(0)
            elif corrcoef_type == 'pos_mean':
...
#         # XXX: bottleneck is here (remove this and put a Mv / index)
#         for icat, cat in enumerate(categories):
...
#         print distances[itest][:10]
#         print sp.dot(corrcoef_kernels, testv)[:10]
        distances[itest] = sp.dot(corrcoef_kernels, testv)
        #raise
